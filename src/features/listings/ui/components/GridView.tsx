            ...
            <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden hover:shadow-md transition-shadow cursor-pointer">
                ...
                <div className="relative h-48 bg-gray-100">
                    ...
                    {item.classificationName && (
                        <span className="absolute top-3 left-3 z-10 inline-flex items-center gap-1 px-2.5 py-1 bg-black/50 backdrop-blur-sm text-white text-xs font-medium rounded-full">
                            🏷 {item.classificationName}
                        </span>
                    )}

                    <span className={`absolute top-3 right-3 z-10 inline-flex px-2.5 py-1 text-xs font-semibold rounded-full ${statusColor}`}>
                        {item.statusLabel}
                    </span>
                </div>
...
export const GridSkeleton = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden animate-pulse">
