...
interface Props {
    isOpen: boolean;
    onClose: () => void;
    listing: Pick<ListingItem, "title" | "city" | "region" | "statusId" | "statusLabel">;
    ...
    return (
        <BaseModal isOpen={isOpen} onClose={handleClose} title="Status Change">
            ...
            <div className="mb-4">
                ...
                <span className={`inline-flex px-3 py-1.5 text-sm font-medium rounded-full ${currentStatus?.color ?? "bg-gray-100 text-gray-600"}`}>
          {listing.statusLabel}
        </span>
            </div>
            ...
            <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                    {STATUS_OPTIONS.filter((s) => s.id !== listing.statusId).map((status) => (
                        <button
                            key={status.id}
                        ...
                        >
              <span className={`inline-flex px-2 py-0.5 rounded-full text-xs ${status.color}`}>
                {status.label}
              </span>
                        </button>
                    ))}
