...
const StatusBadge = ({ status }: { status: PriceChangeRequestDetails["requestStatus"] }) => {
...
export const PriceRequestDetailsModal = ({
  ...
  const { data, isLoading, error } = useGetPriceChangeRequestDetails(
    requestId,
    {
      ...
      initialData: initialData ?? undefined, // ← بيمنع الـ client fetch لو الداتا موجودة
    }
  );
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="mb-5 p-4 bg-gray-50 rounded-xl">
            <p className="text-sm font-semibold text-gray-900">{data.landTitle}</p>
          ...
          <div className="space-y-4 mb-5">
            <div>
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1">Requested On</p>
              <p className="text-sm text-gray-900">{formatDate(data.requestedOn)}</p>
            </div>

            <div>
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1">Requested By</p>
              <p className="text-sm text-gray-900">{data.requestedByName}</p>
            </div>

            <div>
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1">Request Status</p>
              <StatusBadge status={data.requestStatus} />
            </div>

            <div>
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1">Current Price</p>
  ...
  {data?.currentPrice?.toLocaleString() ?? "—"} SAR
            ...
            <div>
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1">Suggested Price</p>
              <p className="text-sm font-semibold text-blue-600">{data.suggestedPrice.toLocaleString()} SAR</p>
            </div>

            <div>
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1">Reduction</p>
              <p className="text-sm font-semibold text-green-600">
                -{data.reductionValue.toLocaleString()} SAR ({data.reductionPercentage.toFixed(1)}%)
              </p>
            </div>

            <div>
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1.5">Reason for Price Change</p>
              <p className="text-sm text-gray-700 leading-relaxed bg-gray-50 rounded-xl px-4 py-3">{data.reason}</p>
            </div>
          </div>
